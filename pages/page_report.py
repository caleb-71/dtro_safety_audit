...
def render():
    ...
    st.caption("자체종합안전심사 AI 분석 결과 보고서를 워드 파일로 생성합니다. (연도는 데이터 기준 자동 설정)")

    st.info("""
    📋 보고서 구성
    - 제1장: 분석 개요
    - 제2장: AI 분류 결과 요약 (차트 + AI 해설)
    - 제3장: 파트별 세부 분석
    - 제4장: 부서별 분석 (AI 해설)
    - 제5장: 리스크 등급 분석
    - 제6장: 조치 이행실태 분석 (이행률·컨설팅 우선 대상)
    - 제7장: 반복 지적사항 분석
    - 제8장: 집중 점검 및 컨설팅 권고 (파트별 AI 컨설팅 의견)

    ℹ️ AI 해설·컨설팅 문단은 Ollama 실행 중일 때 생성됩니다. 꺼져 있으면 통계 표만 포함된 보고서가 생성됩니다.
    """)
    ...
    if st.button(
        ...
        with st.spinner("보고서 생성 중... (AI 해설 포함 시 2~3분 소요)"):
            try:
